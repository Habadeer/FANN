...
ArtificialNeuralNetwork::ArtificialNeuralNetwork(std::vector<uint64_t> networkShape, FunctionParams functionParameters, EActivationFunction function, double lspeed)
{
    learningSpeed = lspeed;
    ActivationFunctionSelection = function;
    par = functionParameters;
    v_NetworkShape = networkShape;
...
ArtificialNeuralNetwork::NeuronLayer::NeuronLayer(uint32_t numberOfNeurons, uint32_t numberOfNeuronsInPreviousLayer, ArtificialNeuralNetwork* networkPtr)
{
    ANN = networkPtr;
    for(uint16_t i = 0; i < numberOfNeurons; i++)
    {
        v_Neurons.push_back(Neuron(numberOfNeuronsInPreviousLayer, networkPtr));
    }
}

ArtificialNeuralNetwork::NeuronLayer::Neuron::Neuron(uint32_t numberOfInputs, ArtificialNeuralNetwork* networkPtr)
{
    ANN = networkPtr;
...
double ArtificialNeuralNetwork::NeuronLayer::Neuron::neuronImpulse(const std::vector<double>& inputs)
{
    ...
    return functionOutput(FUNCTION, inputXWeightsSum);
}

double ArtificialNeuralNetwork::NeuronLayer::Neuron::functionOutput(EFunctionDerivative select, double input)
{
    switch(ANN->ActivationFunctionSelection)
    {
    case LINEAR:
        if(select == 0)
        {
            return ANN->par.a * input + ANN->par.b;
        }
        return ANN->par.a;
    case UNISIGMOID:
        if(select == 0)
        {
            return 1 / (1 + exp(-ANN->par.a * input));
        }
        return (ANN->par.a * exp(-ANN->par.a * input))/(pow(exp(-ANN->par.a * input) + 1, 2));
    case BISIGMOID:
        if(select == 0)
        {
            return (1 - exp(-ANN->par.a * input)) / (1 + exp(-ANN->par.a * input));
        }
        return (2*ANN->par.a * exp(ANN->par.a * input)/pow(exp(ANN->par.a * input) + 1, 2));
    case GAUSS:
        if(select == 0)
        {
            return ANN->par.a * exp(-pow(input - ANN->par.b, 2) / (2 * pow(ANN->par.c, 2)));
        }
        return (-ANN->par.a * (input -ANN->par.b) * exp(-pow((ANN->par.b - input), 2)/(2*pow(ANN->par.c,2))))/pow(ANN->par.c, 2);
    default:
        return 0;
    }
}
...
void ArtificialNeuralNetwork::NeuronLayer::Neuron::calibrateNeuronWeight(std::vector<double> inputs)
{
    ...
    {   
        v_Weights.at(i) += ANN->learningSpeed * m_NeuronError * functionOutput(FUNCTION_DERIVATIVE, v_Weights.at(i) * inputs.at(i)) * inputs.at(i);
    }
    v_Weights.back() += ANN->learningSpeed * m_NeuronError * functionOutput(FUNCTION_DERIVATIVE, v_Weights.back());
}
